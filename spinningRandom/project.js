// 1. deposit money
// 2. Determin the number of lines to be bet on
...
// how many lines do they bet on
const getNumberOfLines = () => {
  while (true) {
    const lines = prompt("Eneter a number of liens to bet on (1 - 3) : ");
...
const getBet = (balance, noOflines) => {
  while (true) {
    const bet = prompt("Eneter total bet per line : ");
    const numberBet = parseFloat(bet);

    if (isNaN(numberBet) || numberBet <= 0 || numberBet > balance / noOflines) {
...
const spin = () => {
  ...
  const reels = []; // each array represent a column
...
const getAWinning = (rows, bet, lines) => {
  let winnings = 0;
...
const game = () => {
  ...
  while (true) {
    ...
    printRows(rows);
    const winnings = getAWinning(rows, bet, numberOfLines);
    balance += winnings;
